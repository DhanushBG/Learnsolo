...

{
  ...
  if ($_FILES["uploadvideo"]["error"] > 0)
    ...
    $target_path="videos/php/";
    $target_path=$target_path.basename($cname);
     if (file_exists("videos/php/" . $_FILES["uploadvideo"]["name"]))
      {
     ...
     move_uploaded_file($_FILES['uploadvideo']['tmp_name'],$target_path);
      ...
      $con=mysql_connect("localhost","root","")or die("cannot connect");
       mysql_select_db("course",$con)or die ("cannot connect to database");
        ...
        $sql="INSERT INTO table_phpvideo(name,type) VALUE('".$cname."','".$type."')"; 
             $result=mysql_query($sql);
             echo" <span class='byline'> <h4>Success</h4></span>";
              echo "<span class='byline'>Your video ".$cname." has been successfully uploaded</span>";
               echo " <span class='byline'>Your Video is Stored in: " . "videos/php/" . $_FILES["uploadvideo"]["name"];
               echo"</span> <hr/>";

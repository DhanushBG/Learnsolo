        ...
        <!-- Nav -->
          <nav id="nav">
            <ul>
              ...
              <li><a href="/projects/course/web/page1/page2/uploadvideos.php">Go Back</a></li>
            </ul>
          </nav>
...

{
  $allowedExts = array("jpg", "jpeg", "gif", "png", "mp3", "mp4", "wma");
    ...
    if ((($_FILES['uploadvideo']['type'] == "video/mp4")
    || ($_FILES['uploadvideo']['type'] == "audio/mp3")
    || ($_FILES['uploadvideo']['type'] == "audio/wma")
    || ($_FILES["uploadvideo"]["type"] == "image/pjpeg")
    || ($_FILES['uploadvideo']["type"] == "image/gif")
    || ($_FILES["uploadvideo"]["type"] == "image/jpeg"))

    && ($_FILES["uploadvideo"]["size"] < 20000000000)
  ...
  if ($_FILES["uploadvideo"]["error"] > 0)
    ...
    $target_path="videos/android/";
    $target_path=$target_path.basename($cname);
     if (file_exists("videos/android/" . $_FILES["uploadvideo"]["name"]))
      {
        echo" <span class='byline'> <h4>Error:</h4>";
      echo $_FILES["uploadvideo"]["name"] . " already exists.</span>";
      echo" <span class='byline'> Please Check the file name and ensure that the videos are uploaded in the correct sequence</span>";
      }
    else
      {
     move_uploaded_file($_FILES['uploadvideo']['tmp_name'],$target_path);
      ...
      $con=mysql_connect("localhost","root","")or die("cannot connect");
       mysql_select_db("course",$con)or die ("cannot connect to database");
        ...
        $sql="INSERT INTO table_andvideo(name,type) VALUE('".$cname."','".$type."')"; 
             $result=mysql_query($sql);
             echo" <span class='byline'> <h4>Success</h4></span>";
              echo "<span class='byline'>Your video ".$cname." has been successfully uploaded</span>";
               echo " <span class='byline'>Your Video is Stored in: " . "videos/android/" . $_FILES["uploadvideo"]["name"];
               echo"</span> <hr/>";

...
{
  $allowedExts = array("jpg", "jpeg", "gif", "png", "mp3", "mp4", "wma","pdf","docx","zip");
  ...
  if ($_FILES["uploadvideo"]["error"] > 0)
    ...
    $target_path="materials/android/";
    $target_path=$target_path.basename($cname);
     if (file_exists("materials/android/" . $_FILES["uploadvideo"]["name"]))
      {
        echo" <span class='byline'> <h4>Error:</h4>";
      echo $_FILES["uploadvideo"]["name"] . " already exists.</span>";
      echo" <span class='byline'> Please Check the file name and ensure that the files are uploaded in the correct sequence</span>";
      }
    else
      {
     move_uploaded_file($_FILES['uploadvideo']['tmp_name'],$target_path);
      ...
      $con=mysql_connect("localhost","root","")or die("cannot connect");
       mysql_select_db("course",$con)or die ("cannot connect to database");
        ...
        $sql="INSERT INTO table_andmat(name,type) VALUE('".$cname."','".$type."')"; 
             $result=mysql_query($sql);
             echo" <span class='byline'> <h4>Success</h4></span>";
              echo "<span class='byline'>Your file ".$cname." has been successfully uploaded</span>";
               echo " <span class='byline'>Your file is Stored in: " . "materials/android/" . $_FILES["uploadvideo"]["name"];
               echo"</span> <hr/>";
